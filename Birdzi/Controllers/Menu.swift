...
class Menu: UIViewController ,UICollectionViewDelegate,UICollectionViewDataSource {
   ...
   // var transition: JTMaterialTransition?
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let rect = CGRect(origin: CGPoint(x: cell.frame.origin.x+20,y :cell.frame.origin.y + self.collectionView.frame.origin.y + self.menuView.frame.origin.y), size: CGSize(width: cell.imgView.frame.size.width, height: cell.imgView.frame.size.height))
